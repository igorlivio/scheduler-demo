...
int main() {
  ...
  for (int i=0; i<n; i++) {
    ...
    i2 = i + 1; //pode ser retirado, mas preferi nao mexer na estrutura já pronta
    mod0 = i2 % a[0];
    mod1 = i2 % a[1];
    mod2 = i2 % a[2];
    mod3 = i2 % a[3];
    mod4 = i2 % a[4];
    //printf("mod 0..4: %d %d %d %d %d i2: %i i1: %i\n", mod0, mod1, mod2, mod3, mod4, i2, i); //for debugging
    if (mod0 == 0) printf("%d", 0); // no printf poder ser substituido por uma respectiva string sem alteração no resultado
    else modsum++;
